...
import {
  ArrowRight, Atom, BookOpen, FlaskConical, Flame, Gamepad2, Play, Sparkles, TrendingUp, Trophy, Zap,
} from "lucide-react";
...
export default function Dashboard() {
  ...
  const continueLessons = [
    { subject: "Fizika", topic: "Kinematika — 2-dars", progress: 35, color: "physics", icon: Atom },
    { subject: "Matematika", topic: "Hosila", progress: 60, color: "math", icon: TrendingUp },
    { subject: "Kimyo", topic: "Oksidlanish", progress: 12, color: "chem", icon: FlaskConical },
  ];
      ...
      <section className="grid grid-cols-1 lg:grid-cols-3 gap-4">
        <div className="paper-card rounded-3xl p-7 lg:col-span-2">
          <span className="pill mb-3">
            Grade {user?.grade} · {user?.goal}
          </span>
          <h1 className="font-serif text-3xl md:text-5xl font-semibold tracking-tight mb-2">
            {t("dash.welcome")},{" "}
            <span className="italic text-primary-soft">{user?.name?.split(" ")[0]}</span>
          </h1>
      ...
      <section>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          {recommendations.map((r, i) => (
            <div
              key={r.title}
              className="paper-card paper-card-hover rounded-2xl p-6 animate-slide-up bg-accent/15 border-accent/30"
              style={{ animationDelay: `${i * 60}ms` }}
            >
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          {quickActions.map((a, i) => (
            <Link
              key={a.to}
              to={a.to}
              className="group paper-card paper-card-hover rounded-2xl aspect-[5/4] flex flex-col items-center justify-center p-6 animate-slide-up"
              style={{ animationDelay: `${i * 60}ms` }}
            >
...
function StatCard({
  icon: Icon, value, label, variant,
}: { icon: any; value: number; label: string; variant: "paper" | "ink" | "accent" }) {
  const styles = {
